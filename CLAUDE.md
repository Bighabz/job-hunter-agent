...
5. **Resume upload**: inject `C:\Users\candidate\Desktop\jobhunter\job-hunter\master\resume.pdf` via JS `DataTransfer` trick (see `uploading-files-via-browser-automation` skill). Confirm filename appears; `input.files.length=0` after injection is EXPECTED.
